...
export { CROSS_VERSION_UPGRADE_EVIDENCE_TYPE, CROSS_VERSION_UPGRADE_SCHEMA_VERSION };
...
type Snapshot = Record<string, { mode: number; sha256: string; symlink?: string }>;
...
async function runCliJson(
  ...
  if (exitCode !== 0) {
    const parsed = stdout.trim() ? parseJson(stdout, "Failed CLI operation") : undefined;
    throw new Error(String(parsed?.error ?? (stderr.trim() || `CLI exited ${exitCode}`)));
  }
  const parsed = parseJson(stdout, "CLI operation");
  if (parsed[successField] !== true) {
    throw new Error(String(parsed.error ?? (stderr.trim() || "CLI operation failed")));
  }
  return parsed;
}
...
async function snapshot(root: string): Promise<Snapshot> {
  ...
  async function visit(directory: string): Promise<void> {
    for (const entry of (await readdir(directory, { withFileTypes: true })).sort((left, right) =>
      ...
      if (relativePath === ".bts/recovery" || relativePath.startsWith(".bts/recovery/")) continue;
...
function managedPlanPaths(plan: UpdatePlanJson): string[] {
  return (plan.files ?? []).flatMap((file) => (typeof file.path === "string" ? [file.path] : []));
}
...
export function protectedCategory(
  ...
  const actionablePaths = (plan.actionable ?? []).flatMap((entry) =>
    typeof entry === "string" ? [entry] : typeof entry?.path === "string" ? [entry.path] : [],
  );
  if (!managedPlanPaths(plan).includes(path) && !actionablePaths.includes(path)) {
...
function assertPlanIdentity(
  ...
  if (
    ...
    throw new Error("Upgrade plan lacks verified source, target, token, or actionable identity");
  }
}
...
async function verifyBuild(fixture: UpgradeFixtureCase, projectDir: string, cliPath: string) {
  ...
  return {
    ...
    steps: result.steps.map((step) => ({
      ...
      step: step.step,
      stderr: typeof step.stderr === "string" ? step.stderr.slice(0, 400) : undefined,
...
function validateSourceReceipt(
  ...
  if (
    ...
    upgradeFixture.fixtureType !== "better-fullstack/executable-upgrade-fixture" ||
    ...
    JSON.stringify(upgradeFixture.caseIds) !== JSON.stringify(REQUIRED_UPGRADE_FIXTURE_CASE_IDS)
  ) {
    throw new Error("Source fixture is not bound to its successful release receipt");
  }
}
...
export async function runCrossVersionUpgradeQualification(options: {
  ...
  const fixture = await loadAndVerifyUpgradeFixtureBundle(options.sourceFixturePath);
  const fixtureSha256 = hash(fixtureBytes);
  validateSourceReceipt(JSON.parse(receiptBytes.toString()), fixture, fixtureSha256);
  ...
  try {
    ...
    if (!isEarlierVersion(fixture.release.version, targetManifest.releaseVersion)) {
      throw new Error(
        `Source ${fixture.release.version} is not older than target ${targetManifest.releaseVersion}`,
      );
    }
    const projectDirectories = await materializeUpgradeFixtureBundle(fixture, projectRoot);
    for (const [index, fixtureCase] of fixture.cases.entries()) {
      const projectDir = projectDirectories[index];
      if (!projectDir) throw new Error(`Materialized fixture ${fixtureCase.id} is missing`);
      let caseActionable = 0;
      let caseCategories: Record<string, number> = {};
      let caseRecoveredExactly = false;
      let caseBuild: CrossVersionCaseResult["build"] = {
        passed: false,
        steps: [],
      ...
      try {
        ...
        const initialPlan = await runCliJson(cliPath, ["update", projectDir, "--json"], "ok");
        assertPlanIdentity(initialPlan, fixture.release.version, targetManifest.releaseVersion);
        // oxlint-disable-next-line no-await-in-loop
        const userEdit = await injectManagedUserEdit(projectDir, managedPlanPaths(initialPlan));
        // oxlint-disable-next-line no-await-in-loop
        ...
        if (userEdit) {
          // oxlint-disable-next-line no-await-in-loop
          plan = await runCliJson(cliPath, ["update", projectDir, "--json"], "ok");
          assertPlanIdentity(plan, fixture.release.version, targetManifest.releaseVersion);
        }
        const protectedAs = userEdit ? protectedCategory(plan, userEdit.path) : null;
        const actionable = plan.actionable ?? [];
        caseActionable = actionable.length;
        caseCategories = plan.summary ?? {};
        if (actionable.length > 0) {
          // oxlint-disable-next-line no-await-in-loop
          const applied = await runCliJson(
            cliPath,
            ["update", projectDir, "--apply", "--review-token", plan.reviewToken ?? "", "--json"],
            "ok",
          );
          if (!applied.recoveryId) throw new Error("Applied update returned no recovery point");
          if (userEdit) {
            // oxlint-disable-next-line no-await-in-loop
            const current = await readFile(join(projectDir, userEdit.path));
            if (!current.equals(userEdit.bytes)) {
              throw new Error(`Apply overwrote the injected user edit: ${userEdit.path}`);
            }
          }
          // oxlint-disable-next-line no-await-in-loop
          await runCliJson(
            cliPath,
            ["recovery", "apply", applied.recoveryId, "--project-dir", projectDir, "--json"],
            "success",
          );
          // oxlint-disable-next-line no-await-in-loop
          const recoveredSnapshot = await snapshot(projectDir);
          caseRecoveredExactly = JSON.stringify(recoveredSnapshot) === JSON.stringify(before);
          if (!caseRecoveredExactly)
          ...
          // oxlint-disable-next-line no-await-in-loop
          const secondPlan = await runCliJson(cliPath, ["update", projectDir, "--json"], "ok");
          assertPlanIdentity(secondPlan, fixture.release.version, targetManifest.releaseVersion);
          // oxlint-disable-next-line no-await-in-loop
          await runCliJson(
            cliPath,
            [
        ...
        if (options.verifyBuilds === false) {
          caseBuild = { passed: true, steps: [], verified: false };
        } else {
          // oxlint-disable-next-line no-await-in-loop -- toolchain verification is intentionally ordered
          caseBuild = await verifyBuild(fixtureCase, projectDir, cliPath);
        }
        if (!caseBuild.passed)
          throw new Error("Updated project did not pass its ecosystem verifier");
        results.push({
          actionable: caseActionable,
          build: caseBuild,
          categories: caseCategories,
          ecosystem: fixtureCase.ecosystem,
          id: fixtureCase.id,
          recoveredExactly: caseRecoveredExactly,
          result: "pass",
          userEdit: userEdit ? { path: userEdit.path, protectedAs: protectedAs ?? "manual" } : null,
        });
      } catch (error) {
        results.push({
          actionable: caseActionable,
          ...
          categories: caseCategories,
          ecosystem: fixtureCase.ecosystem,
          error: boundedError(error, [root, dirname(resolve(options.sourceFixturePath))]),
          id: fixtureCase.id,
          recoveredExactly: caseRecoveredExactly,
          result: "fail",
          userEdit: null,
    ...
    const recoveredCaseCount = results.filter((result) => result.recoveredExactly).length;
    ...
    if (!overallSuccess) {
      throw new Error("Cross-version qualification did not pass every case with recovery evidence");
    }
    return report;
  } finally {
