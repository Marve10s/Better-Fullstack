...
async function main(): Promise<void> {
  ...
  const projectLifecyclePackageJson = JSON.parse(
    await readFile(PROJECT_LIFECYCLE_PACKAGE_JSON_PATH, "utf-8"),
  );
  const originalProjectLifecyclePackageJsonString = await readFile(
    PROJECT_LIFECYCLE_PACKAGE_JSON_PATH,
    "utf-8",
  );
  ...
  try {
    ...
    projectLifecyclePackageJson.version = canaryVersion;
    await writeFile(
      PROJECT_LIFECYCLE_PACKAGE_JSON_PATH,
    ...
    await writeFile(
      PROJECT_LIFECYCLE_PACKAGE_JSON_PATH,
      originalProjectLifecyclePackageJsonString,
    );
    await writeFile(
      TEMPLATE_GENERATOR_PACKAGE_JSON_PATH,
