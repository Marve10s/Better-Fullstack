...
const PROJECT_LIFECYCLE_PACKAGE_JSON_PATH = join(
  process.cwd(),
...
async function main(): Promise<void> {
  ...
  await updateJsonFile(PROJECT_LIFECYCLE_PACKAGE_JSON_PATH, (lifecyclePackageJson) => {
    lifecyclePackageJson.version = newVersion;
  });
  ...
  await $`git add apps/cli/package.json packages/create-bfs/package.json packages/project-lifecycle/package.json packages/types/package.json packages/template-generator/package.json plugin/.codex-plugin/plugin.json plugin/.claude-plugin/plugin.json plugin/plugin.json bun.lock`;
