...
import { readBtsConfig } from "../apps/cli/src/utils/bts-config";
import { getVerifier } from "../testing/lib/verify";
import { installExactReleaseCli } from "./exact-release-cli";
import {
...
type CrossVersionCaseResult = {
  ...
  build: {
    passed: boolean;
    steps: Array<{ step: string; success: boolean; skipped: boolean }>;
    verified: boolean;
  };
  ...
  userEdit: {
    path: string;
    protectedAs: "conflict" | "manual" | "user-edited";
  } | null;
};
...
async function runCliJson(cliPath: string, args: string[]): Promise<UpdatePlanJson> {
  ...
  if (parsed.ok !== true) throw new Error(String(parsed.error ?? "CLI operation failed"));
  return parsed;
}
...
async function injectUserEdit(projectDir: string): Promise<{ bytes: Buffer; path: string } | null> {
  const scaffoldManifest = record(
    JSON.parse(await readFile(join(projectDir, "bts.lock.json"), "utf8")),
  );
  const hashes = record(scaffoldManifest.hashes);
  const relativePath = Object.keys(hashes)
    .sort()
    .find((candidate) => USER_EDIT_MARKERS.has(extname(candidate)));
  if (!relativePath) return null;
  const marker = USER_EDIT_MARKERS.get(extname(relativePath));
  if (!marker) return null;
  await appendFile(join(projectDir, relativePath), marker);
  return { bytes: await readFile(join(projectDir, relativePath)), path: relativePath };
}

function protectedCategory(
  plan: UpdatePlanJson,
  path: string,
): "conflict" | "manual" | "user-edited" {
  ...
  try {
    ...
    for (const [index, fixtureCase] of fixture.cases.entries()) {
      ...
      try {
        ...
        // oxlint-disable-next-line no-await-in-loop
        const plan = await runCliJson(cliPath, ["update", projectDir, "--json"]);
        ...
        if (actionable.length > 0) {
          // oxlint-disable-next-line no-await-in-loop
          const applied = await runCliJson(cliPath, [
            "update",
            projectDir,
            "--apply",
            "--review-token",
          ...
          // oxlint-disable-next-line no-await-in-loop
          recoveredExactly = JSON.stringify(await snapshot(projectDir)) === JSON.stringify(before);
          if (!recoveredExactly) throw new Error("Recovery did not restore exact pre-apply bytes");

          // Reapply from the restored source state so the target build tests upgraded bytes.
          // oxlint-disable-next-line no-await-in-loop
          const secondPlan = await runCliJson(cliPath, ["update", projectDir, "--json"]);
        ...
        if (!build.passed) throw new Error("Updated project did not pass its ecosystem verifier");
        results.push({
        ...
        results.push({
          actionable: 0,
          build: { passed: false, steps: [], verified: options.verifyBuilds !== false },
          categories: {},
          ecosystem: fixtureCase.ecosystem,
    ...
    const report: CrossVersionUpgradeReport = {
      ...
      status: "passed",
      source: { sha: fixture.release.sourceSha, version: fixture.release.version },
      startedAt,
      target: {
