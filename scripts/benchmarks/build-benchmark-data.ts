 ...
 *
 * Usage:
 *   bun run scripts/benchmarks/build-benchmark-data.ts <summary.json> [out.json]
 * (omit out.json to write the dataset to stdout)
 */
import { readFile, writeFile } from "node:fs/promises";

import type { ScaffbenchSummary } from "@scaffbench/index";
...
if (import.meta.main) {
  ...
  if (!inputPath) {
    console.error("usage: bun run scripts/benchmarks/build-benchmark-data.ts <summary.json> [out.json]");
    process.exit(1);
  }
