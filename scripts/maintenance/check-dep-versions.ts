 ...
 *
 * Usage:
 *   bun run scripts/maintenance/check-dep-versions.ts                  # audit all ecosystems
 *   bun run scripts/maintenance/check-dep-versions.ts --ecosystem rust  # audit one ecosystem
 *   bun run scripts/maintenance/check-dep-versions.ts --update          # update stale versions in-place
 *   bun run scripts/maintenance/check-dep-versions.ts --json            # JSON output for CI
 */
...
const NPM_MAP = resolve(ROOT, "packages/template-generator/src/dependencies/add-deps.ts");
