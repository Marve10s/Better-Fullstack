...
- `apps/cli/test/recommendations/cli-builder-sync.test.ts` depends on root-safe path resolution and should fail hard when parsing prompt/schema data instead of silently skipping checks.
...
- Compatibility rules can expose an API option before `packages/template-generator/src/template-handlers/features/api.ts` can emit the matching client/server files. Keep compatibility, handler branches, and `templates/api/<option>/...` coverage in sync.
