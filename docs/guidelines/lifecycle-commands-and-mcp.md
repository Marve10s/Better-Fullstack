...
- Project lifecycle operations are declared once in `apps/cli/src/operations/` with their input
  schema, output schema, safety, and handler. The stdio MCP server is a projection of that
  table; do not register a tool with its own copy of the schema. CLI commands in
  `apps/cli/src/run.ts` still declare their own flag schemas and call the same core modules, so a
  change to an operation's input needs the matching CLI command checked by hand.
