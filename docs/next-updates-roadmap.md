...
| T5.8 | Ship the Devframe-based `devtools` surface from the shared operations table: project panel, HTTP MCP route, static report. See `docs/projects/active/devframe-devtools.md`.                             | Parity test covers every structured operation, the panel proves status and a plan in a browser, and the stdio MCP contract is unchanged.                                        |
