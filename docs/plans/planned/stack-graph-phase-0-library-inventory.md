...
1. **Batch 0 — close the existing asymmetry:** import the already-registered ecosystem categories (§1) in `legacyProjectConfigToStackParts`; extend round-trip tests to cover them. No new roles, no UI work. *This is the recommended first PR.* Caveats found during Phase 0:
...
7. **Batch 6 (Phase 3 entry) —** route `getDisabledReason` library branches through graph checks; demote `compareLegacyConfigToStackParts` to test-only.
