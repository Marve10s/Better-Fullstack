...
7. **Batch 6 (Phase 3 entry) —** partially shipped on PR #220: `compareLegacyConfigToStackParts` is test-only, shared graph compatibility helper data now lives outside `compatibility.ts`, and `getDisabledReason` routes promoted frontend/mobile library branches plus backend Payments/CMS/AI, Java build-tool/library, and shared non-TypeScript email/search/caching/observability disables through graph candidate checks. Remaining work: move unsupported Elixir generated-tool disabled reasons into graph checks and retire the corresponding flat fallbacks.
