...
import { getWebPackagePath, getServerPackagePath } from "@/platform/project-paths";
  ...
  switch (ai) {
    case "vercel-ai":
      ...
      if (
        frontendPath &&
        webFrontend &&
        ["tanstack-router", "react-router", "react-vite", "tanstack-start", "next", "vinext", "redwood"].includes(
          webFrontend,
        )
      ) {
        addPackageDependency({
