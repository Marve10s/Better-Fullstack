...
import type { GeneratedStackPage, PublishedStackSeed } from "@/lib/stack-pages/types";
...
async function generateBasePage(
  ...
  if (optionIssues.length) {
    throw new Error(
      `${seed.slug}: explicitly selected options are disabled:\n${optionIssues.join("\n")}`,
    );
  }
  ...
  if (!generated.success || !generated.tree) {
    throw new Error(
      `${seed.slug}: virtual project generation failed: ${generated.error ?? "unknown error"}`,
    );
  }

  const canonicalParts = deriveCanonicalParts(effectiveSelection, ecosystemParts, seed);
  const architecture = deriveArchitecture(effectiveSelection, canonicalParts, ecosystemParts);
...
async function main() {
  ...
  const pages = generated.map(({ page }, index) => {
    const nearest = generated
      ...
      .sort(
        (a, b) =>
          a.distance - b.distance || b.priority - a.priority || a.slug.localeCompare(b.slug),
      )
      .slice(0, 3)
