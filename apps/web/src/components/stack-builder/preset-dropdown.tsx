  ...
  return (
    <DropdownMenu>
      ...
      >
        ...
        <span className="font-pixel text-[9px] leading-none">{m.presetMenu()}</span>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-64 bg-fd-background">
        {PRESET_TEMPLATES.map((preset) => {
          ...
          return (
            ...
            >
              <div className="font-pixel text-xs">{localizedPreset.name}</div>
              <div className="text-xs text-muted-foreground">{localizedPreset.description}</div>
            </DropdownMenuItem>
          );
        })}
      </DropdownMenuContent>
