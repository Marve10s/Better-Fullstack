...
const ja_changelogrelease20260612highlightbenchmark3 = /** @type {(inputs: Changelogrelease20260612highlightbenchmark3Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`ベンチマークされたフロンティア モデルは、同じプロジェクト仕様を 3 つの方法 (プロンプトのみ、CLI、MCP サーバー) でスキャフォールディングします。MCP パス上のエージェントは、出力トークンが 4 分の 1 で、最大 7 倍の速さで完了しました。完全な結果は、インタラクティブなグラフとともにホームページに表示されます。`)
};
...
const zh_hant1_changelogrelease20260612highlightbenchmark3 = /** @type {(inputs: Changelogrelease20260612highlightbenchmark3Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`用同一組專案 spec 測試前沿模型的三種 scaffold 路徑：純 prompt、我們的 CLI、以及我們的 MCP 伺服器。走 MCP 路徑的代理程式最高快 7×，輸出 tokens 少 4×；完整結果已在首頁透過互動式圖表展示。`)
};
...
const de_changelogrelease20260612highlightbenchmark3 = /** @type {(inputs: Changelogrelease20260612highlightbenchmark3Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Wir haben Frontier-Modelle beim Erstellen derselben Projektspezifikationen auf drei Wegen verglichen: nur per Prompt, über unser CLI und über unseren MCP-Server. Agenten auf dem MCP-Pfad waren bis zu 7× schneller bei 4× weniger Ausgabetokens; die vollständigen Ergebnisse gibt es live auf der Homepage mit einem interaktiven Diagramm.`)
};
...
const uk_changelogrelease20260612highlightbenchmark3 = /** @type {(inputs: Changelogrelease20260612highlightbenchmark3Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Порівняли frontier-моделі, які генерують той самий проєкт трьома шляхами: лише prompt, наш CLI і MCP-сервер. На MCP-шляху агенти завершували до 7× швидше з 4× меншою кількістю output-токенів; повні результати є на головній з інтерактивним графіком.`)
};
