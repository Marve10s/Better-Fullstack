...
`--ai ai-cli` selects AI CLI support. The `aiTooling` Stack Parts add local MCP and skill capabilities. `--ai-docs` controls which repository instruction files are generated.
...
- The MCP and Skills Stack Parts are development-workflow capabilities. They do not replace app authentication, database, or API choices.
...
- If capability setup prompts during scripted runs, use explicit flags like this guide does so defaults are deterministic.
