...
description: "102 runs across Claude Code, Codex CLI, Gemini CLI, Kilo, and opencode, measuring speed, cost, tokens, and whether generated projects install and build."
date: 2026-06-12
authors:
  - Ibrahim Elkamali
tags:
...
Coding agents are very good at writing code and surprisingly bad at _starting_ projects. Ask one to
...
specs, same prompts, same validation. We then added a deliberately [light third sweep](#the-light-sweep-gemini-kilo-and-opencode), nine more models across **Gemini CLI**, **Kilo**, and **opencode**, on the lightest spec only,
...
One important asymmetry: the GPT sweep ran two days earlier, against the generator _before_ we
...
Validation runs _after_ the agent finishes, against whatever it left on disk:
...
- **The structure is vendor-independent.** For every GPT model, MCP is the fastest path and
  ...
  different vendor's models _and_ a different agent CLI.
...
- **GPT-5.5 is the prompt-path outlier.** It was the only model across both sweeps to pass _every_
...
_choosing a configuration_, the file-writing is done by the generator, deterministically. That's
...
_can_ do; it's just the slowest, most expensive, least reliable way to get one.
...
reliably in between. The ranking never changed across paths; the _gap_ did. Tooling is the great
