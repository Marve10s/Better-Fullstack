...
rather than a reshuffle. The difference is *what* the extra thinking fixes: Claude models already
spend their effort verifying as they go (Fable low passes 5 specs in 37 steps), while 5.6's default
budgets produce one-shot projects with unverified dependency facts. Give 5.6 room to check its own
manifests and the hallucinations largely evaporate.
...
- **Stack selection is solved; verified assembly is not.** 88–97% wired at a 9–38% build rate is the
  whole story in one line. "Knows the right library" and "produces a project that builds" are
  different skills — and the gap between them is exactly what a scaffolder closes.
