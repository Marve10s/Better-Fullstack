...
To refresh package-manager registry install evidence, pass the exact current CLI version. Tags such as `latest` are intentionally rejected as non-reproducible evidence.

```sh
CLI_VERSION=$(jq -r .version apps/cli/package.json)
bun run test:published-package --specifier "$CLI_VERSION"
```
