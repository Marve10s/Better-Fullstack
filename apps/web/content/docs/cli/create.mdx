...
`--shape` names what you are building. The CLI then asks which language or platform fits that shape and runs only the prompts that shape needs, so nothing is pinned that you did not choose.
...
Platform and ecosystem flags answer the shape's own question ahead of time. The remaining prompts for that stack still run, so add`--yes` when you need a command that never asks anything:
...
With`--yes` there is nothing to prompt, so a shape falls back to its default: TypeScript for`frontend` and`backend`, React Native for`mobile`.

A shape decides the stack, so it is rejected rather than silently overruled when the rest of the command already decides it too:`--config`,`--from-history`,`--part`, and`--template` each carry a complete stack, a flag the shape switches off contradicts it, and`--shape mobile` accepts selectors for exactly one platform. Kotlin, Swift, and Flutter apps are built by Gradle, XcodeGen, and Flutter, which the CLI does not run, so`--install` is skipped with a notice on those platforms instead of reporting an install that did not happen.

## Templates

`--template` starts from a filled-in stack instead of individual flags. Any flag you pass alongside it overrides that field, and`--yes` accepts the rest of the template as-is.
...
The tables below are generated from the same shared schemas the CLI, Stack Builder, and MCP server use, so accepted values and defaults stay in sync with the generator.
...
`history` lists recent scaffolded projects with the reproducible command that created each one. Use
it to recreate a stack, compare generated output, or share a known-good command.
...
To reuse a stack, run`history`, copy the reproducible command for the one you want, and re-run it
with a new project name or adjusted flags. History is stored locally under your user directory and
does not affect generated project files.
...
- UI libraries, deployment targets, providers, and tooling capabilities are stack-sensitive.
- For a visual check, use the [Stack Builder](https://better-fullstack.dev/new).
