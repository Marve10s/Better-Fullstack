...
Select `shadcn-lint` together with `oxlint` or `eslint` through `--addons`, or use the graph forms `--part codeQuality:universal:oxlint --part codeQuality:universal:shadcn-lint`. Add `prettier` if you choose the ESLint + Prettier profile. The interactive Code Quality prompt accepts one base profile and this optional second check.
