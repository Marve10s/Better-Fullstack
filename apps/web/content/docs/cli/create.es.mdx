...
El comando `create` escribe un proyecto nuevo desde una configuracion de stack tipada. Usalo de forma interactiva para explorar, o pasa flags cuando necesites la misma forma de proyecto cada vez.
...
Con npm, los flags de Better Fullstack van despues de `--`. Los comandos npm generados pueden usar `npx create-better-fullstack@latest`; eso ejecuta el mismo entrypoint del paquete en forma no interactiva.

## Modos del comando

| Modo | Forma del comando | Usalo cuando |
| --- | --- | --- |
...
| `--verify` | Ejecuta checks generados despues del scaffold cuando esta soportado. |
| `--yes` | Acepta defaults. Incompatible con flags centrales del stack. |
| `--yolo` | Salta confirmaciones de seguridad donde esta soportado. |
...
| `--manual-db` | Salta prompts de setup de base de datos especificos de provider cuando aplica. |
