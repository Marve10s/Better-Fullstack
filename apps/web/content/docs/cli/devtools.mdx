...
The report contains the panel plus the result of every read operation for that project, baked at
build time. Serve the directory from any static host. Actions that need a live server, such as
running checks or applying an update, are disabled in a report.
