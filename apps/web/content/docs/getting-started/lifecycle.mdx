...
local edits. Manifest v1 does not prove generator lineage or make an update recoverable.
...
| Apply newer templates to the same stack        | `update`                       | No; apply needs the plan token and risk acknowledgement. |
...
Create an independent recovery point, review the categorized plan and its token, then apply with
both required authorization flags:
...
Local edits, secrets, removed files, and conflicts are not selected for automatic writes. Apply is
destructive, has no built-in rollback, and is not a cross-version safety guarantee.
...
Normal and `--json` output run the same deterministic target matrix, including supported native
ecosystem checks. JSON changes presentation and captures subprocess output; it does not make those
commands read-only. Missing targets, toolchains, or incomplete execution fail the command.
