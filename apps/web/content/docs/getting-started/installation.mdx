...
| .NET         | .NET SDK 10 or newer                                                      |

Multi-ecosystem projects can also include Kotlin Compose, SwiftUI, Flutter, Blazor, or Rust frontend
parts. Install Android/Gradle, Xcode, Flutter, .NET, or Rust tooling only for the app parts you select.
...
The wizard asks for the ecosystem, stack options, dedicated tooling capabilities, Git, and dependency installation.
...
Use `--verify` when you want the CLI to run every generated target check after scaffolding. It does
not start dev servers, but the invoked toolchains may fetch dependencies and write lockfiles,
