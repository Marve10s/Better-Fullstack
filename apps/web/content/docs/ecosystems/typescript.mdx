...
| Mobile storage/testing | `mmkv`, `maestro`, `react-native-testing-library` |
| Mobile services | `expo-notifications` `expo-updates` `expo-linking` |
...
- Mobile options require a native Expo frontend. React Navigation emits an `App.tsx` entrypoint, Expo Router emits file routes, and push/OTA/deep-link choices add Expo config plus helper files.
