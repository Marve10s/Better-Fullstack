...
Multi-Ecosystem mode can also generate standalone `blazor-webassembly` or `blazor-web-app`
frontend parts alongside a backend in another supported language.
...
Use the schema-derived [.NET options reference](/docs/reference/options/dotnet/) for the exhaustive
framework, data, auth, API, testing, jobs, realtime, observability, validation, caching, and deploy
values.
...
`--dotnet-web-framework aspnet-blazor`, which selects the solo .NET backend/application surface.
