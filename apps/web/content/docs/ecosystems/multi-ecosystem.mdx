...
For a supported React web frontend, add these Stack Parts to use shadcn/lint alongside Oxlint:

```text
--part frontend.css:typescript:tailwind
--part codeQuality:universal:oxlint
--part codeQuality:universal:shadcn-lint
```

Use `codeQuality:universal:eslint` and `codeQuality:universal:prettier` for the ESLint + Prettier profile instead of Oxlint. The plugin checks the React application; backend tools such as Ruff or golangci-lint keep their own Role Bindings. See [Code Quality in the builder](/docs/builder/#code-quality) for supported frontends and the generated rule policy.
...
1. **Applications:** choose web, mobile, backend, or a combination. Each card lists the directory it generates, the choices its step offers, and the languages it supports.
2. **One step per application:** choose the application's language, framework, and capabilities. The backend step includes an optional database and its hosting. The mobile step offers React Native, Kotlin, Swift, and Flutter.
...
4. **Review:** name the project, check each application's framework, language, directory, and capabilities, then copy the command, preview the files, run the project in the browser, download a ZIP, or copy a share link.
