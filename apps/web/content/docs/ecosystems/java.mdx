...
Java projects support Spring Boot, Quarkus, Micronaut, and plain Java templates. Choose Java options for build tools, persistence, auth, application libraries, and test libraries.
...
| Category | Values |
| --- | --- |
| Web framework | `spring-boot` `quarkus` `micronaut` `none` |
| Build tool | `maven` `gradle` `none` |
| ORM | `spring-data-jpa` `jooq` `mybatis` `none` |
| Auth | `spring-security` `keycloak` `none` |
| API | `spring-graphql` `openapi-generator` `none` |
| Logging | `logback` `log4j2` `none` |
