...
Project planning, creation, and addition tools include `graphSummary`, `effectiveStack`, and `stackPartSpecs` so agents can inspect the same graph-derived stack view that `bts.jsonc` persists.
</Callout>
...
| Output | JSON success flag plus generated file/directory summary, `graphSummary`, `effectiveStack`, and `stackPartSpecs`. |
...
| Optional inputs | Full project stack selections. |
| Output | JSON success flag, project directory, file count, graph metadata, addon warnings, and install command guidance. |
...
| Output | Existing config summary, proposed additions, graph metadata for both, already-present addons, and compatibility warnings. |
...
| Output | JSON result from the add handler, graph metadata after the update, and install guidance when available. |
