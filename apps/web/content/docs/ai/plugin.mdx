...
The bundle also implements the open Agent Plugins 1.0 format. Its root `plugin.json` identifies the
plugin, `skills/` contains the reusable Agent Skills, and `mcp.json` declares the stdio MCP server.
Compatible clients can therefore load the same package without a client-specific rearrangement.

The existing Codex and Claude Code manifests remain available for their marketplace flows.
