...
describe("Default package manager matrix", () => {
  ...
  describe("explicit package manager installs", () => {
    for (const testCase of EXPLICIT_CASES) {
      it(
        `scaffolds the default --yes path with ${testCase.manager}`,
        async () => {
          const projectDir = join(SMOKE_DIR, `explicit-${testCase.manager}`);
          const expectedFiles = [
            "bts.jsonc",
            "package.json",
            LOCKFILE_BY_MANAGER[testCase.manager],
          ];
          ...
          expect((packageJson.packageManager as string).startsWith(`${testCase.manager}@`)).toBe(
            true,
          );
  ...
  describe("inferred user-agent defaults", () => {
    for (const testCase of INFERRED_CASES) {
      it(
        `infers ${testCase.manager} from npm_config_user_agent`,
        async () => {
