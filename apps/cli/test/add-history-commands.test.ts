...
describe("CLI add command", () => {
  ...
  it(
    "adds Kong with its Compose scaffold and remains idempotent",
    async () => {
      ...
      const sourceCreateResult = await runCli(
        ["create", configSourceName, "--yes", "--no-install", "--no-git", "--disable-analytics"],
        { cwd: root },
      );
  ...
  it(
    ...
    async () => {
      ...
      const [husky, lefthook] = await Promise.all([
        readFile(join(projectDir, ".husky", "pre-commit"), "utf8"),
        readFile(join(projectDir, "lefthook.yml"), "utf8"),
      ]);
  ...
  it(
    ...
    async () => {
      ...
      const retry = await runCli(["add", "--project-dir", projectDir, "--addons", "gitleaks"], {
        cwd: root,
      });
  ...
  it(
    ...
    async () => {
      ...
      expect(config.stackParts).toContainEqual(
        expect.objectContaining({ role: "email", toolId: "resend" }),
      );
...
describe("CLI update command", () => {
  it(
    "rejects write/read-only flag conflicts before touching the project",
    async () => {
...
describe("CLI history command", () => {
  it(
    "prints JSON history and supports clear",
    async () => {
  ...
  it(
    "stores the ecosystem-specific reproducible command for Python projects",
    async () => {
...
describe("CLI telemetry command", () => {
  it(
    "reports status, persists enable/disable, and honors env override precedence",
    async () => {
